...
print('Noise is generated successfully')
...
generator = FDomainDetFrameGenerator(FDomainCBCGenerator, epoch= inject_params['tc'] - 55, detectors=ifos,\
...
print('Data is generated successfully')

#-- setting up the loglikelihood model --

#-- params --
recover_approx = 'IMRPhenomD'                # for likelihoood model
...
variable_params = ['mass1', 'mass2', 'spin1z', 'spin2z', 'ra', 'dec', 'inclination', 'distance', 'tc', 'polarization']
...
def relbin_log_likelihood(q):
    ...
    """
    mchirp, mass_ratio, s1z, s2z, ra, dec, iota, pol, v_com, tc = q 
    m1 = mass1_from_mchirp_q(mchirp, mass_ratio)
    m2 = mass2_from_mchirp_q(mchirp, mass_ratio)
    distance = distance_from_comoving_volume(v_com)
...
if(mass_ratio_center - 0.07 < 1):
    ...
    q_min, q_max = mass_ratio_center - 0.1, mass_ratio_center + 0.1
...
def generate_postSamps(raw_samples, ifos, save_to_hdf=False):
    ...
    if(save_to_hdf):
        ...
        with h5py.File(os.getcwd() + '/post_samples/post_samples_interp_{}_narrow_prior.hdf'.format(name), 'a') as file:
