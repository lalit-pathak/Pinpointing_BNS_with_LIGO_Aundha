...
def plot_corner(samples, fig=None, **kwargs):
    ...
    defaults_kwargs = dict(smooth=0.9)
...
def plot_corner_2datasets(samples, colors, linestyle, measure=None, filename=None, \
    ...
    if(kwargs.get('truth_in_legend')):
        
        lines.append(mpllines.Line2D([0], [0], color=kwargs.get('truth_color'), lw=kwargs.get('hist_kwargs').get('lw')))
