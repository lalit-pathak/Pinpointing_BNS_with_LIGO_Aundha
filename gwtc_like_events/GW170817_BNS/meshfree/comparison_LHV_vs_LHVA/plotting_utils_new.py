...
np.column_stack((mchirp, q, chi_eff, iota, ra, dec, dL)), np.column_stack((mchirp, chi_eff, iota, ra, dec, dL)) 

def pos_samples_modified(filename): # 04/11/23
    ...
    return np.column_stack((mchirp, q, chi_eff, iota, ra, dec, dL)), np.column_stack((mchirp, chi_eff, iota, ra, dec, dL)) 
...
def plot_corner(samples, fig=None, **kwargs):
    ...
    defaults_kwargs = dict(
            ...
            truth_color='#0c0c0d', quantiles=None,
            #levels=[1 - np.exp(-0.74**2/2), 1 - np.exp(-1.32**2/2)],
            levels=[0.5, 0.9],
          #  levels=[0.9],                 #--changes on 04/11/23
...
def title_formats(samps, labels, titles, fmt_arr, measure=None, bins=None, flag=None):
    ...
    range_vals = []
    tlabels = np.empty(7, dtype=object) #-- change this for more parameters
