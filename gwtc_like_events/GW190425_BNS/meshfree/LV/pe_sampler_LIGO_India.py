...
from rbf_pe_utils_LIGO_India import RBFInterpolatedLikelihood, evaluateRBFList, cdfinv_q, prior_transform, pycbc_log_likelihood, relbin_log_likelihood
...
def dynesty_sampler(interp_data, boundary, boundary_Vcom_tc, sampler_params=None, save_to_hdf=False, **params):
    ...
    def prior(cube):
        ...
        return prior_transform(cube, boundary, boundary_Vcom_tc)
    ...
    with open('noise_details.txt', 'a') as fn:
        ...
        fn.write('Evidence:{}'.format(result['logz'][-1]))

    sample_keys = ['Mc', 'mass_ratio', 'chi1z', 'chi2z', 'ra', 'dec', 'iota', 'pol', 'Vcom', 'tc']  # according to prior order chronology
...
def generate_postSamps(raw_samples, ifos, k, save_to_hdf=False):
    ...
    eff_samps_dict.update(dict(pol=eff_samps[:,8], Vcom=eff_samps[:,7], tc=eff_samps[:,9], eta=eta, chi1z=eff_samps[:,2], chi2z=eff_samps[:,3], seed_PE=seed_PE, nodes_seed=nodes_seed))
    ...
    if(save_to_hdf):
        ...
        with h5py.File(os.getcwd() + '/post_samples/post_samples_interp_{}_{}.hdf'.format(name, k), 'a') as file:
            
            for key in params:

                file.create_dataset(key, data=eff_samps_dict[key])
            
#             group = file_eff.create_group('injection_{}'.format(count))
            
#             i = 0  # reduntant
            
#             for key in params:

#                 group.create_dataset(key, data=eff_samps_dict[key])
                   
#         file = h5py.File(os.getcwd() + '/post_samples/post_samples_interp_seedPE_{}_nodes_seed_{}_{}.hdf'.format(seed_PE, nodes_seed, name), 'w')   
#         i = 0
#         for key in params:
