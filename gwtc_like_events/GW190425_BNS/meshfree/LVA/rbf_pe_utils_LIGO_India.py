...
#-- Creating Simulated data from Injection parameters --

def generate_sim_data(inj_approx, fLow, segLen, ifos, inj_params, noise_tilde, psd):
    
    """
    Function to generate simulated GW data
    
    Parameters
    ----------
    inj_approx : Waveform Approximant for Injection
    fLow       : lower_frequency_cutoff          (Hz)
    ifos       : list of interferometers
    inj_pararms: injection parameters (type: numpy array)
    noise_tilde: frequency domain noise dictionary (dictionary with 'ifo' as keys)
    psd        : frequency domain PSD dictionary (dictionary with 'ifo' as keys)
    
    Returns
    -------
    data       : frequency domain GW data (type: python dictionary)   
    """
    ...
    m1, m2, s1z, s2z, iota, dL, ra, dec, pol, tc = inj_params
    
    #-- generating the Detector Frame signal -- 

    static_params_gen = {'approximant': inj_approx, 'f_lower': fLow, 'mass1': m1, 'mass2': m2,\
                         'spin1z': s1z, 'spin2z': s2z , 'polarization': pol, 'inclination': iota, 'tc': tc,\
                         'distance': dL, 'ra': ra, 'dec': dec}

    generator = FDomainDetFrameGenerator(FDomainCBCGenerator, epoch=static_params_gen['tc'] - 310 , detectors=ifos,\
...
# def generate_sim_data(inj_approx, fLow, segLen, ifos, inj_params, noise_seed):
...
#             psds[ifo] = load_frequencyseries(os.getcwd() + '/PSD_Virgo_10Hz_to_2048Hz_nonzero.txt')

#         elif(ifo=='K1'):

#             psds[ifo] = load_frequencyseries(os.getcwd() + '/PSD_KAGRA_10Hz_to_2048Hz_nonzero.txt')
            
#         elif(ifo=='A0'):
            
#             psds[ifo] = load_frequencyseries(os.getcwd() + '/PSD_O4_10Hz_to_2048Hz_nonzero.txt')

#         else:

#             psds[ifo] = load_frequencyseries(os.getcwd() + '/PSD_O5_10Hz_to_2048Hz_nonzero.txt')
            
#     #-- generate noise from psds --
    
#     i = 0
#     j = 10   # noise_seed for LIGO India will always be 10 greater than the 'count' or 'noise_seed' 
    
#     #-- Treating the noise realizations this way will give us a comparable situation 'where first we compare LHVK (with a particular set of noise realizations) and then LHVKA (with LHVK having the same set of noise realizations as the previous case but A1 will have a new noise realization) --
...
def get_cleaned_data_psd(event, ifos, analysis_start_time, analysis_end_time, high_pass_cutoff, \
    ...
    for ifo in ifos:
        ...
        # ts = merger.strain(ifo).time_slice(merger.time - analysis_start_time, merger.time + analysis_end_time)
        
        ts = read_frame("{}-{}_LOSC_CLN_4_V1-1187007040-2048.gwf".format(ifo[0], ifo),
                        '{}:LOSC-STRAIN'.format(ifo),
                       start_time=merger.time - analysis_start_time,   
                       end_time=merger.time + analysis_end_time,      
...
def hdf_to_txt(event, filename, ifos):
    ...
    filename: hdf file containig psd (saved using extract_noise_psds function)
    ifos: list of ifos
...
def generate_covmat(fiducial_params, ifos, segLen, event):
    ...
    for ifo in ifos:
        ...
        if(ifo=='V1'):
            ...
            ifo_mod = 'Virgo'
            LVKA_detectors[ifo_mod]['psd_path'] = os.getcwd() + '/psd/{}_PSD_{}.txt'.format(event, ifo)
            ...
            LVKA_detectors[ifo]['psd_path'] = os.getcwd() + '/psd/{}_PSD_{}.txt'.format(event, ifo)
    ...
    with open('Fisher_SNR_{}.txt'.format(name), 'a') as fisher:
        
        fisher.write('SNR: %.2f'%SNR + '\n')
        

    totF = myLVKA_Net.FisherMatr(myEvent_dict, df= 1/segLen, use_m1m2=True) # replace df=1/360 by an appropriate variable name
...
#-- subroutine to generate rbf nodes ---
def generate_nodes(fiducial_params, nodes_params, ifos, fLow, segLen, event):
    ...
    if(nodes_params['nodes_gauss_num'] != 0):
        ...
        cov_mat = generate_covmat(fiducial_params, ifos, segLen, event)
        ...
        temp = np.random.multivariate_normal(mu, cov_mat_Mc_q, size=10000000)
...
def createRBFInterpolants(nodes, sigmasq, zc, times, rbf_params, nodes_params, saveinterpolants=True):
    ...
    interp_data = {'interpolants': interpolants, 'basis_vectors': basis_vectors, 'coeffs_matrix': coeffs_matrix, \
                   'times': times, 'nodes': nodes, 'nodes_params': nodes_params}
...
def start_up(fiducial_params, data_params, waveform_params, rbf_params, nodes_params, \
                                       ifos, nProcs, segLen, event, savetrainingdata=False, saveinterpolants=False, verbose=False):
    """
    ...
    st = time.time()
    nodes = generate_nodes(fiducial_params, nodes_params, ifos, waveform_params['fLow'], segLen, event)
    et = time.time()
...
def RBFInterpolatedLikelihood(q, interpolants, basis_vectors, times, **params):
    ...
    mchirp, mass_ratio, s1z, s2z, ra, dec, iota, pol, Vcom, tc = q 
    ...
    z = redshift_from_comoving_volume(Vcom) #--
    distance = distance_from_comoving_volume(Vcom)
...
def prior_transform(cube, boundary, boundary_Vcom_tc):
    ...
    cube[8] = boundary_Vcom_tc['Vcom'].min() + (boundary_Vcom_tc['Vcom'].max() - boundary_Vcom_tc['Vcom'].min()) * cube[8] 
#     cube[8] = np.power((boundary_dL_tc['dL'].max()**3 - boundary_dL_tc['dL'].min()**3)*cube[8] + boundary_dL_tc['dL'].min()**3, 1./3) # distance: unifrom prior in dL**3
    cube[9] = boundary_Vcom_tc['tc'].min() + (boundary_Vcom_tc['tc'].max() - boundary_Vcom_tc['tc'].min()) * cube[9]                   # tc: uniform prior
