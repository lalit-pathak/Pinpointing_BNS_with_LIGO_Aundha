...
injection_data = np.loadtxt('injections_Net_SNR_20_to_40_{}.txt'.format(combination))

#-- defining a variable to store the total no. of injections --

total_injections = len(injection_data[:400])
...
for count in range(total_injections):
    ...
    ra, dec, dL, z, trigTime = np.loadtxt('injections_Net_SNR_20_to_40_{}.txt'.format(combination))[count, :5]  # (Here '5' means it will go  till 4th indexed column)
