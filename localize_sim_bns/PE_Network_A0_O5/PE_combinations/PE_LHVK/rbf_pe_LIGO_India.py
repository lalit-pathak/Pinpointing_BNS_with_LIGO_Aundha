...
noise_param = 1    #-- this value will change and be saved for every combination (say LHVK or LHV or HLV) [to make sure of uncorrelated guassian noise in each detector]
...
for count in range(total_injections):
    ...
    ra, dec, dL, z, trigTime = np.loadtxt('injections_Net_SNR_20_to_25_L1H1V1K1.txt')[count, :5]  # (Here '5' means it will go  till 4th indexed column)
...
#         f.write('nodes_seed: %d, start_up_time: %.2f minutes'%(nodes_seed, (et_start_up-st_start_up)/60))
    ...
    with open('seeds_and_timings_rbf.txt', 'a') as f:
        ...
        f.write('\n' + 'injection: %d, rbf_PE_time: %.2f minutes'%(count, (et_PE-st_PE)/60))
